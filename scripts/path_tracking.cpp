...
public:
      ...
      : Node("trajectory_tracker")
  {
    ...
    declare_parameter("ki", 1.0);

    // calculate timer step
    std::chrono::milliseconds timer_step = (std::chrono::milliseconds)(int)(1000.0 / get_parameter("rate").as_double());

    // create publishers and subscribers
    pub_cmd_vel_ = create_publisher<geometry_msgs::msg::Twist>("cmd_vel", 10);

    // subscriber to odometry (feedback)
    sub_odom_ = create_subscription<nav_msgs::msg::Odometry>("odom", 10, std::bind(&TrajectoryTracker::odomCallback, this, std::placeholders::_1));

    // create main loop timer
    timer_ = create_wall_timer(timer_step, std::bind(&TrajectoryTracker::timerCallback, this));
    ...
    // path subscriber
    sub_path_ = create_subscription<nav_msgs::msg::Path>("plan", qos_, std::bind(&TrajectoryTracker::planCallback, this, std::placeholders::_1));
  }
  ...
  void timerCallback()
  {
      ...
      if (received_path && counter < traj.poses.size() - 1)
    {
      ...
      else
      {
        ...
        arma::vec result = (eye - log3 / 2.0 + (1.0 / theta - 1.0 / std::tan(theta / 2.0) / 2.0) * log3 * log3 / theta) * trans_vec;
      ...
      RCLCPP_INFO(this->get_logger(), "%f %f %f %ld %ld", x_err[2], x_err[3], x_err[4], traj.poses.size(), counter);
      ...
      arma::vec p_term = x_err * get_parameter("kp").as_double();
      ...
      arma::vec i_term = int_x_err * get_parameter("ki").as_double();
