...
class node_maker(Node):
    ...
    pid_x = Pid(15.0, 0, 0.2)
    pid_y = Pid(15.0, 0, 0.2)
    pid_w = Pid(15.0, 0, 0.2)
    ...
    def onOdom_data(self, msg: Odometry):
        ...
        if self.finish is not True and self.count < self.plan_d.__len__() - 1 :

            global log, log3, x_err, v_d, omg

            current_config = self.transform_to_matrix(self.pose_to_transform(msg.pose.pose))        

            reference_config = self.transform_to_matrix(self.pose_to_transform(self.plan_d[self.count].pose))

            next_config = self.transform_to_matrix(self.pose_to_transform(self.plan_d[self.count + 1].pose))

            tf_diff_matrix = np.dot(np.linalg.inv(current_config), reference_config)
            ...
            self.get_logger().info(str(x_err[2]) + " " + str(x_err[3]) + " " + str(x_err[4]) + " " + str(self.count))

            # finish logaritma

            tf_diff_matrix = np.dot(np.linalg.inv(reference_config), next_config)
        ...
        elif self.count == self.plan_d.__len__() -1:
